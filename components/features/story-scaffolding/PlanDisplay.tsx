...
const ChapterComponent: React.FC<ChapterComponentProps> = ({ chapter, pageHandlers, isActionLoading }) => {
    ...
    return (
        <div className="mb-4 bg-gray-800/30 rounded-lg border border-gray-700 overflow-hidden">
            <button onClick={() => setIsExpanded(!isExpanded)} className="w-full flex justify-between items-center p-4 bg-gray-700/50 hover:bg-gray-700/80 transition-colors">
                <div>
            ...
            {isExpanded && (
                <div className="p-4 space-y-4">
                    {chapter.pages.length > 0 ? chapter.pages.map(page => <PageComponent key={page.id} page={page} chapterId={chapter.id} handlers={pageHandlers} isActionLoading={isActionLoading} />) : <p className="text-center py-8 text-gray-500">This chapter has no pages yet.</p>}
                </div>
            )}
        </div>
    );
