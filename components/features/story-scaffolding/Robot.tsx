...
const Robot: React.FC<RobotProps> = ({ robotState, mousePosition }) => {
    ...
    return (
        <motion.div className="fixed bottom-0 left-1/2 -translate-x-1/2 z-50 pointer-events-none" style={{ x: x - window.innerWidth / 2, y: y - window.innerHeight }} initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                ...
                <svg width="120" height="120" viewBox="0 0 100 100">
                    ...
                    {robotState === 'writing' && (
                     ...
                     {robotState === 'illustrating' && (
                        <motion.g key="illustrating" initial={{ y: 10, opacity: 0, rotate: -20, x: 10 }} animate={{ y: 0, opacity: 1, rotate: 10, x: 0 }} exit={{ y: 10, opacity: 0, rotate: -20 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
                            <circle cx="75" cy="75" r="6" fill="#6B7280" />
                            <g transform="translate(65, 55) rotate(45)"><PaintBrushIcon className="w-8 h-8 text-purple-400" /></g>
                        </motion.g>
                    )}
                    </AnimatePresence>
                </svg>
